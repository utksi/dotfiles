...
function n_descramble(nparam, js)
	...
	local compound = function(ntab, str, alphabet)
		if ntab ~= n or type(str) ~= "string" or type(alphabet) ~= "string" then
			return true
		end
		local input = {}
	...
	local trans = {
		...
		unid = {
			func = function()
				vlc.msg.dbg(
					"Couldn't apply unidentified YouTube video throttling parameter transformation, aborting descrambling"
				)
				return true
			end,
			match = {},
	...
	while datac and datac ~= "" do
		...
		if string.match(datac, "^function%(") then
			...
			if el == trans.compound.func or el == trans.compound1.func or el == trans.compound2.func then
				datac = string.match(datac, '^.-},e%.split%(""%)%)},(.*)$') or string.match(datac, "^.-},(.*)$")
			else
	...
	for ifunc, itab, args in string.gmatch(script, "c%[(%d+)%]%(c%[(%d+)%]([^)]-)%)") do
		...
		if type(func) ~= "function" or type(tab) ~= "table" or func(tab, arg1, arg2) then
			vlc.msg.dbg(
				"Invalid data type encountered during YouTube video throttling parameter descrambling transformation chain, aborting"
			)
...
function probe()
	return (
		(vlc.access == "http" or vlc.access == "https")
		and (
			(
				(
			...
			) or string.match(vlc.path, "^consent%.youtube%.com/")
		)
	)
end
...
function parse()
	if string.match(vlc.path, "^consent%.youtube%.com/") then
		...
		if not url then
			vlc.msg.err(
				"Couldn't handle YouTube cookie consent redirection, please check for updates to this script or try disabling HTTP cookie forwarding"
			)
			return {}
		end
	...
	then -- This is the HTML page's URL
		...
		while true do
			...
			-- "ytplayer.config" ...
			if string.match(line, "ytInitialPlayerResponse ?= ?{") or string.match(line, "ytplayer%.config") then
		...
		if not path and get_url_param(vlc.path, "el") ~= "detailpage" then
			...
			if video_id then
				path = vlc.access
					...
					.. video_id
					.. "&el=detailpage"
					.. copy_url_param(vlc.path, "fmt")
					.. copy_url_param(vlc.path, "jsurl")
